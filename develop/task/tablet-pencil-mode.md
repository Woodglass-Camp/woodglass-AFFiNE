...
- 平板 Pencil 模式实现
  ...
  - 新增 Widget 包：blocksuite/affine/widgets/tablet-pencil-mode
    ...
    - src/index.ts：注册 `affine-tablet-pencil-mode-widget`，不渲染 UI，仅挂事件：
      - touch/pointerDown → PanTool（panning），并阻止默认；
      - touch/click → 阻止默认（避免选中）；
      - pen/pointerDown → 若当前工具非 BrushTool，则切换到 BrushTool；
      - pen/click → 若当前工具非 BrushTool，阻止默认（不允许非绘图点击）。
...
1. 设置 > Woodglass 功能：
   - 开启“平板 Pencil 模式”：Edgeless 下，手指仅平移、手写笔绘制。
...
- HMR/端口：
  - dev 端支持 `PORT`、`WDS_SOCKET_HOST`、`WDS_SOCKET_PORT` 环境变量；
  - 可使用 `DEV_HTTPS=true SSL_CRT_FILE/SSL_KEY_FILE` 直接以 HTTPS 启动 dev。
- 剪贴板：
  - 建议 https 或 localhost 访问以启用 `navigator.clipboard`；
  - 非安全上下文会导致剪贴板事件不挂载。
