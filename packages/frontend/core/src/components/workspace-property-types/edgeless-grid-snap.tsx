...
export const EdgelessGridSnapValue = ({ readonly }: PropertyValueProps) => {
  ...
  const onChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      if (readonly) return;
      docService.doc.record.setProperty(
        'edgelessGridSnap',
...
export const EdgelessGridSnapDocListProperty = ({
  doc,
}: {
  doc: DocRecord;
}) => {
  ...
  return (
    <StackProperty icon={<GridIcon />}>
      {enabled
        ? (t[
            'com.affine.page-properties.property.edgelessGridSnap.enabled'
          ]?.() ?? t['Enabled']?.())
        : (t[
            'com.affine.page-properties.property.edgelessGridSnap.disabled'
          ]?.() ?? t['Disabled']?.())}
    </StackProperty>
  );
};
